...
#' @description Although functions like `data.table::transpose()` or `base::t()`
#'   can transpose dataframes well, they don't preserve row and column names.
#'   The current function transposes dataframes while preserving row and column
#'   names.
...
transpose_df <- function(df,
  ...
  # assign the row and column names of the original dataframe to the new
  # dataframe
  colnames(t_df) <- rownames(df)
  rownames(t_df) <- colnames(df)

  # convert the row names into a new column and convert it to a tibble
  # dataframe
  t_df <- t_df %>%
